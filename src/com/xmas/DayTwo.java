...
public class DayTwo {
  ...
  Predicate<String> isValidPasswordPuzzle1 = s -> {
    ...
    var matcher = Pattern.compile(patternString).matcher(s);
    if(matcher.find()) {
      int min = Integer.parseInt(matcher.group(1));
      int max = Integer.parseInt(matcher.group(2));
      String charachter = matcher.group(3);
      String password = matcher.group(4);
      var numberOfChars = password.chars().filter(letter -> letter == charachter.charAt(0)).count();
      if (numberOfChars <= max && numberOfChars >= min ) {
        return true;
      }
      return false;
    }
  ...
  static String input = "13-14 f: ffffffffnfffvv\n"
